...
function processJson(data) {
  $('#inner_tbl').html(this.buildTable(data));
}
...
function buildArray(a) {
  let document = this.document
  var e = document.createElement('table'), d, b, c = !1, p = !1, m = {}, h = - 1, n = 0, l; l = '';
  if (0 == a.length) return '<div></div>';
  d = e.insertRow(- 1);
  for (var f = 0; f < a.length; f++) if ('object' != typeof a[f] || isArray(a[f])) 'object' == typeof a[f] && isArray(a[f]) ? (b = d.insertCell(h), b.colSpan = 2, b.innerHTML = '<div class="td_head"></div><table style="width:100%">' + $(buildArray(a[f]), !1).html() + '</table>', c = !0) : p || (h += 1, p = !0, b = d.insertCell(h), m.empty = h, b.innerHTML = '<div class=\'td_head\'>&nbsp;</div>');
  else for (var k in a[f]) l = '-' + k,
    l in m || (c = !0, h += 1, b = d.insertCell(h), m[l] = h, b.innerHTML = '<div class=\'td_head\'>' + encodeText(k) + '</div>');
  c || e.deleteRow(0);
  n = h + 1;
  for (f = 0; f < a.length; f++) if (d = e.insertRow(- 1), td_class = isEven(f) ? 'td_row_even' : 'td_row_odd', 'object' != typeof a[f] || isArray(a[f])) if ('object' == typeof a[f] && isArray(a[f])) for (h = m.empty, c = 0; c < n; c++) b = d.insertCell(c),
    b.className = td_class,
    l = c == h ? '<table style="width:100%">' + $(buildArray(a[f]), !1).html() + '</table>' : ' ',
    b.innerHTML = '<div class=\'' + td_class + '\'>' + encodeText(l) +
    '</div>';
  else for (h = m.empty, c = 0; c < n; c++) b = d.insertCell(c),
    l = c == h ? a[f] : ' ',
    b.className = td_class,
    b.innerHTML = '<div class=\'' + td_class + '\'>' + encodeText(l) + '</div>';
  else {
    for (c = 0; c < n; c++) b = d.insertCell(c),
      b.className = td_class,
